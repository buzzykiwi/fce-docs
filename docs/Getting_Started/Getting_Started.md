...
6. Set up the trigger. The "Trigger on New Records" trigger could use SQL input such as: `SELECT id, full_name, email FROM User ORDER BY updated_at desc LIMIT 5` to take the latest 5 records, and trigger on any new ones found. You only need to set two fields:
   ...
   2. Set the De-duplication to "1. Disable de-duplication", for testing. If you run this trigger live, you will probably need to clear this field, allowing only brand new records to pass through. Alternatively, you may wish to set it to "2. Allow updated records in addition to new records" to allow both new and updated records to pass through to subsequent actions.<p align="center"><img alt="Disable Deduplication" src="../../img/disable-deduplication.png" width="500px"></p>
